...
libs <- c("tidyverse", "RODBC", "keyring")
...
database_afsc <- "afsc"
username_afsc <- keyring::key_list(database_afsc)$username 
password_afsc <- keyring::key_get(database_afsc, keyring::key_list(database_afsc)$username) 
channel_afsc <- odbcConnect(database_afsc, uid = username_afsc, pwd = password_afsc, believeNRows=FALSE)

database_akfin <- "akfin" 
username_akfin <- keyring::key_list(database_akfin)$username 
password_akfin <- keyring::key_get(database_akfin, keyring::key_list(database_akfin)$username) 
...
spp %>% 
  ...
  write_csv(paste0(dat_path, "/bsai_orox_spp_lookup.csv"))
...
catch_clean %>% write_csv(paste0(dat_path, "/bsai_orox_catch_2003_", YEAR, "_confidential.csv"))

observed %>% 
  mutate(thorny = ifelse(species == 350, "SST", "other thornyhead"),
         fmp_subarea = ifelse(between(nmfs_area, 500, 540), "BS", "AI")) %>% 
  write_csv(paste0(dat_path, "/obs_thornyheads_2003_", YEAR, "_confidential.csv"))

prop_sst %>% write_csv(paste0(dat_path, "/prop_sst_catch_2003_", YEAR, ".csv"))
